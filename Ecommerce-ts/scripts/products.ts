import { deleteProduct, IProduct } from './apis.js';
import { cart } from './cart.js';
export const getProductCard = (product: IProduct) => {
    const productItem = document.createElement('div');
    productItem.className = 'group relative overflow-hidden';
    const link = document.createElement('a');
    link.href = `/Ecommerce-ts/product/?id=${product.id}`;
    ...
    productDetails.href = `/Ecommerce-ts/product/?id=${product.id}`;
    ...
    button.addEventListener('click', (e) => {
        ...
        if (!target) return;
        const buttonElement = target.closest('button.add-to-cart') as HTMLButtonElement | null;
...
export const generateStars = (rating: number, count: number) => {
